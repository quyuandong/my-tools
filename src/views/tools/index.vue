<template>
  <div class="page" v-if="dataList.length">
    <div class="navigation-tools">
      <dl>
        <dd v-for="(nav, navindex) in dataList" :key="navindex" :id="navindex" :class="{ nav_active: navindex == curIndex }">
          <a :href="`#${nav.id}`" @click="toNav(navindex)">{{ nav.name }}</a>
        </dd>
      </dl>
    </div>
    <div class="tools-box">
      <section v-for="(tool, index) in dataList" :key="index">
...
import allModule from './index'

//动态加载图片
console.log('allModule: ', allModule);

const dataList = computed(()=>allModule)
